...
class TokenPurchase(Base):
    ...
    payment_method: Mapped[str] = mapped_column(Text, nullable=False, default="stripe")  # 'stripe' | 'btcpay'
    payment_ref: Mapped[str] = mapped_column(Text, unique=True, nullable=False)  # Stripe session id or BTCPay invoice id
...
def generate_api_key() -> tuple[str, str, str]:
    ...
    key_prefix (first 12 chars) is stored for display purposes.
    """
...
async def get_user_by_key_prefix(session: AsyncSession, prefix: str) -> User | None:
    """Fetch users whose key starts with prefix (fast pre-filter before bcrypt check)."""
...
async def get_active_trial_tokens(session: AsyncSession, user_id: int) -> int:
    """Sum of remaining_tokens across non-expired trial grants."""
    from datetime import datetime, timezone
...
async def deduct_tokens(
    ...
    Commits within the session — caller must commit.
    """
    now = datetime.now(timezone.utc)
...
async def total_available_tokens(session: AsyncSession, user: User) -> int:
    """Paid balance + active trial tokens."""
