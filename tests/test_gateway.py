...
async def test_register_new_user(client: AsyncClient):
    r = await client.post("/register", data={"email": "new@example.com"}, follow_redirects=True)
    assert r.status_code == 200
...
async def test_dashboard_bad_key_redirects(client: AsyncClient, db_session):
    r = await client.get("/dashboard?key=sk-notavalidkey12345678901234567890", follow_redirects=False)
    assert r.status_code in (302, 307)


# ── Admin panel ──────────────────────────────────────────────────────────────

async def test_admin_bad_key_forbidden(client: AsyncClient, db_session):
    r = await client.get("/admin?key=wrong")
    assert r.status_code == 403


async def test_admin_valid_key(client: AsyncClient, db_session):
    import os
